...
router.get("/", async (_req: Request, res: Response) => {
  try {
    ...
    const propMap: Record<string, string> = Object.fromEntries(allProperties.map(p => [p.id, p.name]));

    // Group expenses by work order
    const expensesByWO: Record<string, number> = {};
    allExpenses.forEach(e => {
      if (e.workOrderId) expensesByWO[e.workOrderId] = (expensesByWO[e.workOrderId] || 0) + e.amount;
    });
    ...
    allWorkOrders.forEach(wo => {
      ...
      if (wo.price && wo.paid) monthly[month].revenue += wo.price;
...
router.put("/work-orders/:id/paid", async (req: Request, res: Response) => {
  try {
    const { paid } = req.body;
