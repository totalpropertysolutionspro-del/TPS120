...
router.get("/", async (req, res) => {
    try {
        const { propertyId, staffId, date } = req.query;
        let allAssignments = await db.select().from(staffAssignments).all();
        if (propertyId)
            allAssignments = allAssignments.filter(a => a.propertyId === propertyId);
        if (staffId)
            allAssignments = allAssignments.filter(a => a.staffId === staffId);
        if (date)
            allAssignments = allAssignments.filter(a => a.date === date);
...
//# sourceMappingURL=staff-assignments.js.map