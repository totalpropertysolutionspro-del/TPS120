...
type FormState = { name: string; type: "management_company" | "direct"; contactName: string; contactEmail: string; contactPhone: string; notes: string };

interface ClientCardProps {
  c: ClientWithProperties;
  navigate: (page: Page, extra?: { propertyId?: string }) => void;
...
function ClientCard({ c, navigate, expanded, setExpanded, openEdit, handleDelete }: ClientCardProps) {
  return (
    <div className="bg-white rounded-xl shadow-sm border border-gray-100">
      ...
      {(c.contactName || c.contactEmail || c.contactPhone) && (
        <div className="px-4 pb-3 -mt-1 flex items-center gap-2">
          ...
          <ContactButtons email={c.contactEmail} phone={c.contactPhone} />
        </div>
      )}
...
function ClientModal({ editClient, form, setForm, onClose, handleEdit, handleAdd }: ClientModalProps) {
  return (
...
export default function Clients({ navigate }: Props) {
  ...
  const [form, setForm] = useState<FormState>({ name: "", type: "management_company", contactName: "", contactEmail: "", contactPhone: "", notes: "" });
  ...
  const handleClose = () => { setShowAdd(false); setEditClient(null); resetForm(); };
    ...
    <div className="space-y-6 max-w-3xl">
      ...
      {mgmt.length > 0 && (
        <div>
          ...
          <div className="space-y-3">
            {mgmt.map(c => <ClientCard key={c.id} c={c} navigate={navigate} expanded={expanded} setExpanded={setExpanded} openEdit={openEdit} handleDelete={handleDelete} />)}
          </div>
        </div>
      )}
      ...
      {direct.length > 0 && (
        <div>
          ...
          <div className="space-y-3">
            {direct.map(c => <ClientCard key={c.id} c={c} navigate={navigate} expanded={expanded} setExpanded={setExpanded} openEdit={openEdit} handleDelete={handleDelete} />)}
          </div>
        </div>
      )}
